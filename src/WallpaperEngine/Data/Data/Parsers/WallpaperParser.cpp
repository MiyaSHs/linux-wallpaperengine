...
ObjectList WallpaperParser::parseObjects (const JSON& objects, const Project& project) {
    ...
    // Catch per-object so a single broken/unsupported object (Text, Light, scripted property
    // referencing 'engine' global, alignment string going through vec parser, etc.) doesn't
    // abort loading the whole wallpaper. The error is logged and the object is skipped.
    for (const auto& cur : objects) {
	try {
	    result.emplace_back (ObjectParser::parse (cur, project));
	} catch (const std::exception& e) {
	    sLog.error ("Skipping object due to parse error: ", e.what ());
	}
    }
