...
Properties ProjectParser::parseProperties (const std::optional<JSON>& data) {
	...
	try {
	    ...
	    // A single broken property (typically one with a script that uses an unsupported
	    // global like 'engine' or whose scripted value can't be coerced) shouldn't kill
	    // the whole wallpaper load. Skip it and log.
	    sLog.error ("Skipping property '", cur.key (), "' due to parse error: ", e.what ());
	}
    }
