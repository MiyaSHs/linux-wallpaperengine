...
struct ImageData {
    ...
    /** Name of the attachment in the parent's puppet skeleton this image hangs off
     *  of, when set. WPE puppet `.mdl` files carry an MDAT section listing named
     *  attachment points (each with a local transform); a child image with this
     *  field tracks that attachment as its parent moves/animates. */
